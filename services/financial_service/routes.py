...
def get_financial_summary(user_id):
    ...
    recurring_expenses = (
        session.query(RecurringExpense).filter_by(user_id=user_id).all()
    )
    session.close()
        ...
        jsonify(
            {
                ...
                "recurring_expenses": recurring_expense_data,
            }
        ),
        200,
    )
...
@financial_blueprint.route(
    "/<int:user_id>/financials/affordability", methods=["POST"]
)
def perform_affordability_analysis(user_id):
    ...
    monthly_payment = (
        (price - down_payment) / loan_term if loan_term else price
    )
    ...
    return (
        jsonify(
            {"monthly_payment": monthly_payment, "affordable": affordable}
        ),
        201,
    )

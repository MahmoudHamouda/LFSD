...
from shared.config import SECRET_KEY, OAUTH_CLIENT_ID, OAUTH_CLIENT_SECRET, OAUTH_REDIRECT_URI
...
def register():
    ...
    cursor.execute(
        ...
        (email, hashed_password, first_name, last_name, datetime.datetime.utcnow())
    )
    conn.commit()
    conn.close()
...
def login():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, password, account_status FROM Users WHERE email = %s", (email,))
    ...
    return jsonify({"status": "success", "access_token": token, "expires_in": 3600}), 200
...
def oauth_callback():
    ...
    token_response = requests.post(token_url, headers=headers, data=urlencode(data))
    ...
    if not access_token:
        logger.error("OAuth callback failed: Access token not found in response")
    ...
    if user_info_response.status_code != 200:
        logger.error("OAuth callback failed: Failed to retrieve user information")
        return jsonify({"error": "Failed to retrieve user information"}), 400

    user_info = user_info_response.json()
    email = user_info.get("email")
    if not email:
        logger.error("OAuth callback failed: Email not found in user information")
    ...
    if not user:
        cursor.execute(
            ...
            (email, user_info.get("given_name"), user_info.get("family_name"), datetime.datetime.utcnow())
        )
        conn.commit()
        cursor.execute("SELECT user_id FROM Users WHERE email = %s", (email,))
        user = cursor.fetchone()
...
def logout():
    ...
    try:
        ...
        cursor.execute(
            ...
            (decoded["user_id"], token.split(" ")[1], datetime.datetime.utcnow(), decoded["exp"])
        )
        conn.commit()
        conn.close()
        logger.info(f"User logged out successfully: user_id {decoded['user_id']}")
...
def validate_token():
    ...
    try:
        decoded = jwt.decode(token.split(" ")[1], SECRET_KEY, algorithms=["HS256"])
        logger.info(f"Token validation successful: user_id {decoded['user_id']}")
        return jsonify({"status": "success", "message": "Token is valid", "user_id": decoded["user_id"]}), 200
    except jwt.ExpiredSignatureError:

...
class User(Base):
    ...
    # Relationships
    viv_indexes = relationship(
        "VivIndex", back_populates="user", cascade="all, delete-orphan"
    )
    life_goals = relationship(
        "LifeGoal", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    health_daily_summaries = relationship(
        "HealthDailySummary", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    mobility_trips = relationship(
        "MobilityTrip", back_populates="user", cascade="all, delete-orphan"
    )
    viv_logs = relationship(
        "VivLog", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    limit_overrides = relationship(
        "UserLimitOverride", back_populates="user", cascade="all, delete-orphan"
    )
    statements = relationship(
        "Statement", back_populates="user", cascade="all, delete-orphan"
    )
    financial_scores = relationship(
        "FinancialScore", back_populates="user", cascade="all, delete-orphan"
    )
    time_scores = relationship(
        "TimeScore", back_populates="user", cascade="all, delete-orphan"
    )
    health_data_samples = relationship(
        "HealthDataSample", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    conversations = relationship(
        "DBConversation", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    feature_interests = relationship(
        "FeatureInterest", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    time_profile = relationship(
        "TimeProfile",
        uselist=False,
        back_populates="user",
        cascade="all, delete-orphan",
    )
    time_events = relationship(
        "TimeEvent", back_populates="user", cascade="all, delete-orphan"
    )
    ...
    activity_feed = relationship(
        "ActivityFeed", back_populates="user", cascade="all, delete-orphan"
    )
    investment_portfolios = relationship(
        "InvestmentPortfolio", back_populates="user", cascade="all, delete-orphan"
    )
    lifestyle_events = relationship(
        "LifestyleEvent", back_populates="user", cascade="all, delete-orphan"
    )

    # Health & Scores Links
    health_connections = relationship(
        "DBHealthConnection", back_populates="user", cascade="all, delete-orphan"
    )
    health_metrics = relationship(
        "DBHealthMetric", back_populates="user", cascade="all, delete-orphan"
    )
    health_indexes = relationship(
        "DBUserIndex", back_populates="user", cascade="all, delete-orphan"
    )
    health_insights = relationship(
        "DBHealthInsight", back_populates="user", cascade="all, delete-orphan"
    )
    health_settings = relationship(
        "DBHealthSettings", back_populates="user", cascade="all, delete-orphan"
    )
    scores = relationship(
        "DBUserScore", back_populates="user", cascade="all, delete-orphan"
    )
...
class LifeGoal(Base):
    ...
    )  # Renamed from deadline for consistency

    type = Column(
        String, nullable=True
    )  # car, house, emergency_fund, travel, education, custom
...
class Statement(Base):
    ...
    transactions = relationship(
        "FinancialTransaction", back_populates="statement", cascade="all, delete-orphan"
    )
...
class FinancialTransaction(Base):
    ...
    id = Column(String, primary_key=True, default=generate_uuid)
    account_id = Column(
        String, ForeignKey("financial_accounts_v2.id"), nullable=True, index=True
    )  # Made nullable to allow statement-only transactions initially
    statement_id = Column(
        String, ForeignKey("statements.id"), nullable=True, index=True
...
class VivLog(Base):
    ...
    context_snapshot_json = Column(
        JSON, nullable=True
    )  # Values of Finance/Health/Time at decision
...
class DBConversation(Base):
    ...
    id = Column(String, primary_key=True, default=generate_uuid)
    user_id = Column(
        String, ForeignKey("users_v2.id"), nullable=True, index=True
    )  # Optional for now, but suggested for usage tracking
...
class DBMessage(Base):
    ...
    user_id = Column(
        String, ForeignKey("users_v2.id"), nullable=True, index=True
    )  # Tracking which user sent it (or system)
...
class Order(Base):
    ...
    transaction_id = Column(
        String, ForeignKey("transactions_v2.id"), nullable=True
    ...
    )  # PENDING, CONFIRMED, FAILED, COMPLETED, CANCELLED
    ...
    details_json = Column(
        JSON, nullable=True
    )  # Full payload: start/end lat, driver info, etc.
...
class OnboardingSession(Base):
    ...
    data_json = Column(
        JSON, nullable=True
    )  # Stores parsed statement data, income, etc.
    created_at = Column(DateTime, default=datetime.utcnow)
...
class FinancialScore(Base):
    ...
    data_sources_json = Column(
        JSON, nullable=True
    )  # {"statements": true, "onboarding": true}
...
class FeatureInterest(Base):
    ...
    )  # e.g., "whoop_integration", "apple_health"

...
class DBHealthMetric(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "ix_health_metrics_user_type_time", "user_id", "metric_type", "timestamp"
        ),
    )
...
class DBUserIndex(Base):
    ...
    # Metadata
    time_window = Column(
        Enum(TimeWindow), default=TimeWindow.LAST_30_DAYS, nullable=False
    )
...
class DBHealthSettings(Base):
    ...
    sync_frequency = Column(
        Enum(SyncFrequency), default=SyncFrequency.DAILY, nullable=False
    )

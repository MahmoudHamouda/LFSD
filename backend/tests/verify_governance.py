...
sys.path.append(
    os.path.abspath("c:\\Users\\hmahm\\OneDrive\\Desktop\\LFSD Codebase\\LFSD\\backend")
)
...
def test_governance_flow():
    ...
    try:
        ...
        user = User(
            id=user_id,
            email=f"test_gov_{user_id[:8]}@example.com",
            hashed_password="pw",
        )
        db.add(user)
        db.commit()
        ...
        assert (
            booking["success"] == True or booking.get("mock") == True
        ), "Booking failed"
        ...
        assert (
            ...
            or "mock" in str(booking)
        ), f"Status mismatch: {order.status}"
        ...
        if order.transaction_id:
            txn = (
                ...
                .filter(Transaction.id == order.transaction_id)
                .first()
            )
        ...
        audit = (
            db.query(AuditLog)
            .filter(AuditLog.entity_id == order.id, AuditLog.action == "CONFIRM_ORDER")
            .first()
        )
        if not audit:
            # Maybe it failed?
        ...
        print("\nTesting Idempotency...")
        booking_2 = loop.run_until_complete(
            aggregator.book_ride(
                user_id=user_id,
                provider="uber",
                ride_type="UberX",

...
async def run_simulation():
    ...
    try:
        ...
        for sc in scenarios:
            ...
            print(
                f"State Set: Financial={ind['financial']}, Health={ind['health']}, Time={ind['time']}"
            )
            ...
            try:
                response = await service._route_intent(
                    intent_data, context, TEST_USER_ID
                )
                ...
                if response and response.get("type") == "mobility_options":
                    ...
                    if rec:
                        print(
                            f"RECOMMENDATION: {rec['provider']} {rec['type']} - {rec['reasoning']}"
                        )
                    else:

...
def verify_mobility():
    ...
    try:
        ...
        service.connection_service.get_connections = lambda uid: [ConnectionMock('uber', 'connected')]
        ...
        if trip:
            logging.info(f"SUCCESS: Trip {trip.id} found. Provider: {trip.provider}, Cost: {trip.cost_amount}")
            print("SUCCESS_TRIP")
        ...
        # Verify VivLog
        viv_log = db.query(VivLog).filter(VivLog.user_id == user_id).order_by(VivLog.timestamp.desc()).first()
        if viv_log:

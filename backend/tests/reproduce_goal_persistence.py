...
def verify_goal_persistence():
    ...
    try:
        ...
        idx = VivIndex(id="idx-1", user_id=test_user_id, financial_score=50.0, health_score=50.0, time_score=50.0)
        db.add(idx)
        ...
        goal = db.query(LifeGoal).filter(
            LifeGoal.user_id == test_user_id, 
            LifeGoal.target_amount == 9999.0
        ).first()

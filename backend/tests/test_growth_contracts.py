...
def mock_get_current_user():
    ...
    try:
        ...
        if not user:
            user = User(
                email="test_growth@helm.com", id="user_growth_test", role="user"
            )
            db.add(user)
            db.commit()
            db.refresh(user)
...
def test_get_entitlements_free():
    ...
    assert (
        data["usage"]["ai_chat_calls"] >= 1
    ), f"Usage should be at least 1, got {data.get('usage')}"


def test_subscribe_plus():
    response = client.post(
        "/api/growth/subscribe",
        json={"plan_id": "tier_plus", "payment_method_id": "pm_test"},
    )
    assert response.status_code == 200, f"Failed: {response.text}"
...
def test_subscribe_pro():
    response = client.post(
        "/api/growth/subscribe",
        json={"plan_id": "tier_pro", "payment_method_id": "pm_test"},
    )
    assert response.status_code == 200, f"Failed: {response.text}"

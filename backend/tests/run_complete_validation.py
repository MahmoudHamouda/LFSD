    ...
    try:
        ...
        if "user_seed_status" in result:
            ...
            if result.get("user_seed_status") == "SUCCESS":
                ...
                print(
                    f"  ⏳ Deployment detected, user_seed_status: {result.get('user_seed_status')}"
                )
                break
        else:
...
print(
    f"{'✅' if login_summary['passed'] > 0 else '❌'} Authentication: {login_summary['passed']}/{len(test_users)} users authenticated"
)
print(
    f"{'✅' if e2e_success else '❌'} E2E API Tests: {'PASSED' if e2e_success else 'FAILED/INCOMPLETE'}"
)

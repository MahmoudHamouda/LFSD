...
def test_goal_limit_enforcement():
    ...
    for i in range(5):
        resp = client.post("/api/finance/goals", json={
            "title": f"Goal {i+1}",
            "target_amount": 1000,
            "target_date": "2025-12-31T00:00:00"
        })
        assert resp.status_code == 200, f"Failed to create allowed goal {i+1}: {resp.text}"
        print(f"Created Goal {i+1}")

    # 2. Try to Create 6th Goal (Should Fail)
    print("Attempting to create 6th goal (Expected Failure)...")
    resp_fail = client.post("/api/finance/goals", json={
        "title": "Goal 6",
        "target_amount": 1000
    })
    ...
    resp_sub = client.post("/api/growth/subscribe", json={"plan_id": "tier_pro", "payment_method_id": "pm_card"})
    assert resp_sub.status_code == 200
    print("Upgrade Successful.")

    # 4. Try to Create 6th Goal Again (Should Success)
    print("Attempting to create 6th goal again (Expected Success)...")
    resp_success = client.post("/api/finance/goals", json={
        "title": "Goal 6 (Pro)",
        "target_amount": 1000
    })
    ...
    if resp_success.status_code == 200:
        ...
        print(f"❌ Failed! Expected 200, got {resp_success.status_code}: {resp_success.text}")
        sys.exit(1)

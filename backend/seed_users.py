...
def create_base_user(user_id, email, name, persona_type, bio, auth0_id=None):
    ...
    existing = (
        db.query(User)
        .filter((User.email == email) | (User.auth0_id == auth0_id))
        .first()
        if auth0_id
    ...
    if existing:
        print(
            f"   User {email} already exists with ID {existing.id}. Will seed data for this ID."
        )
...
def safe_seed_users():
    ...
    try:
        ...
        fin_result = calculate_financial_health_score(
            user2.id, finance_onboarding, db, is_manual_mode=False
        )
        ...
        health_score = (
            health_result.get("score", 50) if isinstance(health_result, dict) else 50
        )
        ...
        print(
            f"   Calculated scores - Financial: {financial_score}, Health: {health_score}, Time: {time_score}"
        )
        db.add(
            VivIndex(
                user_id=user2.id,
        ...
        user3 = create_base_user(
            "user-health", "health@helm.com", "Health User", "Athlete", "Training."
        )
        ...
        fin3 = calculate_financial_health_score(
            user3.id, health_onboarding, db, is_manual_mode=False
        )
        ...
        db.add(
            VivIndex(
                ...
                health_score=(
                    health3.get("score", 88) if isinstance(health3, dict) else 88
                ),
        ...
        user4 = create_base_user(
            "user-time", "time@helm.com", "Time User", "Pro", "Productivity."
        )
        ...
        fin4 = calculate_financial_health_score(
            user4.id, time_onboarding, db, is_manual_mode=False
        )
        ...
        db.add(
            VivIndex(
                ...
                health_score=(
                    health4.get("score", 50) if isinstance(health4, dict) else 50
                ),
        ...
        user5 = create_base_user(
            "user-super", "super@helm.com", "Super User", "Legend", "All stars."
        )
        ...
        fin5 = calculate_financial_health_score(
            user5.id, super_onboarding, db, is_manual_mode=False
        )
        ...
        db.add(
            VivIndex(
                ...
                health_score=(
                    health5.get("score", 95) if isinstance(health5, dict) else 95
                ),

...
def check_goal_limit(
    ...
    try:
        ...
        if not entitlements or not entitlements.limits:
            ...
            limits_dict = (
                entitlements.limits if isinstance(entitlements.limits, dict) else {}
            )
        ...
        current_count = (
            db.query(func.count(LifeGoal.id)).filter(
                LifeGoal.user_id == current_user.id
            )
        ...
        if current_count >= limit:
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "limit": limit,
                    "suggested_action": (
                        f"Upgrade to {next_plan} for more goals."
                        if next_plan

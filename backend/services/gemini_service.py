...
from services.logic_engine import (
    validate_financial_goal as validate_financial_goal_legacy,
)
...
class GeminiService:
    def __init__(self, db: Session):
        ...
        )  # Updated to 2.5 (2.0 deprecated March 2026)
    ...
    async def get_ride_estimates(
        ...
        raise NotImplementedError(
            "Use mobility_aggregator.compare_prices with valid user_id instead."
        )
    ...
    async def _extract_intent(
        ...
        has_currency = any(
            c in text_lower
            for c in ["$", "aed", "savings", "for a", "$", "dollar", "aed"]
        )
        has_digits = any(char.isdigit() for char in text_lower)

        if (has_save and has_digits) or any(
            k in text_lower
            for k in ["save to plan", "add to goals", "set a goal", "savings goal"]
        ):
        ...
        if any(keyword in text_lower for keyword in car_keywords):
            if not any(
                q in text_lower
                for q in ["how much", "what is", "spending on", "cost of", "do i spend"]
            ):
                logger.info(
                    f"Deterministic Intent: Detected 'mobility.car_purchase' from text: {text}"
                )
        ...
        if any(
            ...
            if any(t in text_lower for t in ["hours", "minutes", "tomorrow", "week"]):
                logger.info(
                    f"Deterministic Intent: Detected 'schedule_event' from text: {text}"
                )
                return {
        ...
        try:
            ...
            if isinstance(data, dict) and data.get("intent", "").startswith(
                "mobility_"
            ):
                reconstructed_slots = self._reconstruct_slots_from_history(
                    history, text
                )
    ...
    async def _handle_car_purchase_request(
        self, intent_data: Dict, context: Dict, user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        if score:
            ...
            expenses = (score.total_monthly_expenses or 0.0) + (
        ...
        target_payment_cap = min(
            available_monthly * 0.15, max(0, available_monthly - safety_buffer)
        )
        ...
        filtered_options = [
            ...
            if o["monthly_cost_value"] <= target_payment_cap * 1.5
        ]

        # If filtering is too aggressive (empty), return cheapest
        if not filtered_options:
            filtered_options = sorted(
                all_options, key=lambda x: x["monthly_cost_value"]
            )[:3]
        else:
    ...
    async def _route_intent(
        ...
        if intent == "mobility.car_purchase":
            try:
                return await self._handle_car_purchase_request(
                    intent_data, context, user_id
                )
            except Exception as e:
                logger.error(f"Error handling car purchase request: {e}", exc_info=True)
                return {
                    "type": "text",
                    "text": "I encountered an error looking up car options. Please try again later.",
                }
        ...
        if intent == "general_conversation" or intent == "needs_clarification":
            ...
            if any(
                k in text_lower
                for k in ["uber", "careem", "cab", "ride", "taxi", "book "]
            ):
                ...
                return await self._handle_mobility_request(
                    intent_data, full_context, user_id
                )

            # Fallback 2: Calendar
            if any(
                k in text_lower for k in ["schedule", "calendar", "meeting", "remind"]
            ):
                logger.info("Fallback: Forcing Calendar Intent")
                intent_data["intent"] = "schedule_event"
                return await self._handle_calendar_request(
                    intent_data, context, user_id
                )
            ...
            if intent == "needs_clarification":
                return {
                    "type": "text",
                    "text": intent_data.get("entities", {}).get(
        ...
        if intent and (intent.startswith("mobility_") or intent == "get_bookings"):
            ...
            if intent.startswith("mobility_") and not intent_data.get("destination"):
                ...
                if from_to:
                    ...
                    intent_data["destination"] = from_to.group(2).strip()
                    logger.info(
                        f"Regex Entity Fix: Start={intent_data['start_location']}, Dest={intent_data['destination']}"
                    )
                elif to_from:
                    intent_data["destination"] = to_from.group(1).strip()
                    intent_data["start_location"] = to_from.group(2).strip()
                    logger.info(
                        f"Regex Entity Fix: Dest={intent_data['destination']}, Start={intent_data['start_location']}"
                    )
                else:
                    ...
                    if to_only and ("book" in text_lower or "check" in text_lower):
                        intent_data["destination"] = to_only.group(1).strip()
                        logger.info(
                            f"Regex Entity Fix (To Only): {intent_data['destination']}"
                        )
    ...
    def _reconstruct_slots_from_history(
        self, history: List[Dict[str, str]], current_text: str
    ) -> Dict[str, str]:
        """
        ...
        for text in reversed(all_user_messages):
            ...
            if from_to:
                if "start_location" not in slots:
                    slots["start_location"] = from_to.group(1).strip()
                if "destination" not in slots:
                    slots["destination"] = from_to.group(2).strip()
            elif to_from:
                if "destination" not in slots:
                    slots["destination"] = to_from.group(1).strip()
                if "start_location" not in slots:
                    slots["start_location"] = to_from.group(2).strip()
            ...
            if to_only and not slots.get("destination"):
                ...
                if any(
                    k in text_lower
                    for k in ["go", "ride", "book", "trip", "travel", "take me"]
                ):
    ...
    async def _handle_mobility_request(
        self, intent_data: Dict, context: Dict, user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            connections = self.connection_service.get_connections(user_id)
            active_providers = [
                c.provider for c in connections if c.status == "connected"
            ]
            ...
            if intent_data["intent"] == "mobility_price_check" or (
                intent_data["intent"] == "mobility_booking" and not provider
            ):
                if not destination:
                ...
                if all_options:
                    ...
                    options_sorted = sorted(
                        all_options,
                        key=lambda x: (
                            float(str(x["estimate"]).split()[-1].replace(",", ""))
                            if "AED" in str(x["estimate"])
                            else 999
                        ),
                    )
                    ...
                    if indices.get("time", 50) < 30:
                        reasoning = (
                            "Your schedule is tight; recommending the fastest provider."
                        )
                    elif indices.get("health", 50) < 30:
                        reasoning = (
                            "You seem low on energy; recommending a comfort ride."
                        )
            ...
            # 3. Handle Booking
                ...
                if not all([destination, provider, ride_type]):
                    return {
                        "type": "text",
                        "text": f"I need a destination, provider, and ride type to book.",
                    }

                import hashlib

                idempotency_key = hashlib.sha256(
                    f"{user_id}:{provider}:{ride_type}:{destination}:{datetime.utcnow().date()}".encode()
                ).hexdigest()
                ...
                booking = await self.mobility_aggregator.book_ride(
                    ...
                    start_location={
                        "lat": start_lat,
                        "lng": start_lng,
                        "address": start_location or "Current Location",
                    },
                    end_location={
                        "lat": end_lat,
                        "lng": end_lng,
                ...
                return {
                    "type": "error",
                    "text": f"Booking failed: {booking.get('error')}",
                }
            ...
            return {
                "type": "error",
                "text": "I had trouble handling your mobility request.",
            }
    ...
    async def _handle_financial_request(
            ...
            try:
                ...
                total_spend = (
                    ...
                    .filter(
                        ...
                        Transaction.amount
                        > 0,  # Simple assumption: positive is spend? Or need type? Assuming debit is positive for now or check 'debit'
                    )
                    .scalar()
                    or 0.0
                )
                ...
                if (
                    total_spend == 0
                    and not self.db.query(FinancialTransaction)
                ...
                breakdown_query = (
                    ...
                    .group_by(FinancialTransaction.category_primary)
                    .all()
                )
                ...
                return {
                    "type": "error",
                    "text": "I had trouble accessing your financial records.",
                }

            if category != "all":
                # Filter for specific category
                filtered = [
                    b for b in breakdown if category.lower() in b["category"].lower()
                ]
                if filtered:
        ...
        return {
            "type": "text",
            "text": "I can help you track your spending and balances. What would you like to know?",
        }

    async def _handle_financial_advisory(
        self, intent_data: Dict, context: Dict, user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        amount = intent_data.get("amount") or intent_data.get("entities", {}).get(
            "amount"
        )

        if amount:
            context_data = {"monthly_income": income, "fixed_expenses": fixed_costs}
            try:
        ...
        try:
            ...
            return {
                "type": "financial_advisory",
                "text": advice,
                "data": {"balance": balance, "advice": advice},
            }
        except Exception as e:
    ...
    async def _handle_calendar_request(
        ...
        if minutes_match:
            ...
            if not hours_match:
                duration_hours = 0  # If only minutes specified
        ...
        try:
            ...
            events = await self.calendar_service.list_events(
                user_id=user_id, time_min=start_dt, time_max=end_dt
            )
    ...
    def _load_viv_context(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user:
                ...
                return {
                    ...
                    "preferences": {
                        ...
                        "trade_off_rule": "balanced_living",
                    },
                    "crisis_mode": False,
                }
            ...
            if preferences.get(
            ...
            ):  # Default to True if not set, or False? Let's default True for now as per "Health" vertical
                health_summary = (
                ...
                if health_summary:
                    health_data = {
                        "sleep_hours": (health_summary.sleep_duration_minutes or 0)
                        / 60.0,
            ...
            return {
                ...
                "life_goals": [
                    {
                        ...
                        "deadline": (
                            getattr(g, "deadline", None).isoformat()
                ...
                "preferences": (
                    user.viv_preferences
                    if user.viv_preferences
                    else {
                        "risk_tolerance": "medium",
                        "trade_off_rule": "balanced_living",
                    }
                ...
                "confidence_scores": {
                    "health": (
                        1.0 if health_data else 0.5
                    ),  # Low confidence if data missing
                    "financial": 1.0,  # Placeholder, would come from Bank API analysis
                },
                "financial_snapshot": {
                    "total_balance": sum(
                        acc.current_balance for acc in user.financial_accounts
                    ),
    ...
    def _simulate_impact(self, intent_data: Dict, viv_context: Dict) -> Dict[str, Any]:
        ...
        if intent_data.get("intent") == "financial_spend":
            ...
            if amount > current_savings_buffer:
                for goal in viv_context["life_goals"]:
                    if goal["priority"] == "high":
                        impact["goal_impact"].append(
                            f"Delays '{goal['goal_name']}' by approx 1 week."
                        )

            # 3. Hard Financial Guardrail (Debt Prevention)
            # Check actual balance from snapshot
            total_balance = viv_context.get("financial_snapshot", {}).get(
                "total_balance", 0
            )
            if amount > total_balance:
                impact["goal_impact"].append(
                    "CRITICAL: This purchase exceeds your total balance. Debt risk likely."
                )
                impact["crisis_override"] = (
                    True  # Flag as crisis to ensure user sees it
                )
            ...
            if viv_context.get("crisis_mode"):
                ...
                if (
                    viv_context["viv_indexes"]["health"] < 30
                    and impact["index_impact"]["health"] > 0
                ):
                    impact["crisis_override"] = True
                    impact["goal_impact"].append(
                        "CRISIS OVERRIDE: Health takes priority over savings."
                    )
    ...
    def _normalize_facts(self, impact_data: Dict, viv_context: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "goal_impact_analysis": (
                " ".join(impact_data["goal_impact"])
                if impact_data["goal_impact"]
                else "No significant impact on long-term goals."
            ),
            "crisis_override": impact_data["crisis_override"],
        }

    def _log_interaction(
        self, user_id: str, intent_data: Dict, response_data: Dict, viv_context: Dict
    ):
        """
    ...
    async def generate_response(
        self, history: List[Dict[str, str]], context: Dict[str, Any]
    ) -> str:
        """
                    ...
                    return json.dumps(
                        {
                            ...
                            "text": _gov.consent_required_message(),
                            "usage": {"input_tokens": 0, "output_tokens": 0},
                        }
                    )
                last_message = _gov.redact(last_message)
        ...
        try:
            ...
            logger.info(
                f"[ORCH] Instance OK. Calling process_message with: '{last_message[:80]}'"
            )
            human_answer, metadata = await orchestrator.process_message(
                last_message, user_id, context, history=history
            )
            logger.info(
                f"[ORCH] process_message returned: is_orchestrator={metadata.get('is_orchestrator')}, intent={metadata.get('intent')}, answer_len={len(human_answer) if human_answer else 0}"
            )
            ...
            if metadata.get("is_orchestrator") and human_answer:
                logger.info(
                    f"Orchestrator cleanly handled the {metadata.get('intent')} intent."
                )
                ...
                response_data = {
                    ...
                    "pipeline": {
                        "tier": 0,
                        "execution_id": "orchestrator-" + str(_uuid.uuid4()),
                    },
                }
                return json.dumps(response_data)
            else:
                logger.info(
                    f"[ORCH] Not handled (is_orchestrator={metadata.get('is_orchestrator')}). Falling through to pipeline."
                )
        except Exception as e:
            logger.error(
                f"[ORCH] Orchestrator Layer CRASHED: {type(e).__name__}: {e}",
                exc_info=True,
            )
        ...
        try:
            ...
            if not history:
                logger.info(
                    "History is empty in generate_response. Using default greeting."
                )
                last_message = "Hello"
            else:
                last_message = (
            ...
            if intent_data.get("intent") in ["financial_report", "financial_balance"]:
                ...
                financial_context["net_worth"] = self.finance_service.get_net_worth(
                    user_id
                )
                ...
                final_response = {"type": "text", "text": response.text}
                ...
                if amount == 0.0:
                    ...
                    if matches:
                        ...
                        if valid_amounts:
                            amount = max(
                                valid_amounts
                            )  # Assume largest number is the target
                        elif matches:
                ...
                if "honda" in text:
                    title = "Honda Civic Plan"
                elif "nissan" in text:
                    title = "Nissan Sunny Plan"
                elif "toyota" in text:
                    title = "Toyota Plan"
                ...
                try:
                    ...
                    financial_context = {
                        ...
                        "fixed_expenses": summary.get(
                            "recurring_bills", 0.0
                        ),  # Proxy for fixed
                    }
                except Exception as e:
                    logger.error(
                        f"Failed to fetch financial context for goal validation: {e}"
                    )
                ...
                if validation_error:
                    final_response = {"type": "error", "text": validation_error}
                    self._log_interaction(
                        user_id, intent_data, final_response, viv_context
                    )
                    return json.dumps(final_response)
                ...
                response_data = await self._handle_calendar_request(
                    intent_data, full_context, user_id
                )
...
"""
            ...
            try:
                ...
                final_response = {
                    ...
                    "text": data.get(
                        "message_body",
                        data.get("summary", "Here is your wellbeing analysis."),
                    ),
                    "data": data,
                    "usage": {
                        "input_tokens": (
                ...
                logger.warning(
                    f"JSON synthesis parsing failed: {e}. Falling back to text."
                )
                pass
            ...
            logger.error(traceback.format_exc())
            print(
                f"ERROR IN GENERATE_RESPONSE [{error_id}]: {type(e).__name__}: {str(e)}"
            )
            return json.dumps(
                {
                    "type": "error",
                    ...
                    "debug_error": (
                        f"{type(e).__name__}: {str(e)}" if settings.DEBUG else None
                    ),
                }
            )
    ...
    async def parse_bank_statement(
        ...
        try:
            with open("debug_log.txt", "a") as f:
                f.write("DEBUG: Preparing content parts\n")
            content_parts = [{"mime_type": mime_type, "data": file_content}, prompt]
            ...
            try:
                ...
                with open("debug_log.txt", "a") as f:
                    f.write("ERROR: Gemini API call timed out after 30 seconds\n")
            ...
            with open("debug_log.txt", "a") as f:
                f.write(f"ERROR: Gemini parsing failed: {e}\n")

...
def calculate_time_score(
    db: Session, user_id: str, window_days: int = 30
) -> Optional[TimeScore]:
    """
...
def _calculate_structure(events: List[CalendarEvent], window_days: int) -> float:
    ...
    for d, daily_events in days.items():
        ...
        start_hours.append(
            first_event.start_time.hour + first_event.start_time.minute / 60.0
        )
...
def _calculate_load(events: List[CalendarEvent], window_days: int) -> float:
    ...
    avg_hours_per_day = (
        total_duration_hours / window_days
    )  # Denominator is window, not active days
...
def _calculate_stress(events: List[CalendarEvent], window_days: int) -> float:
    ...
    if avg_meeting_hours > 5:
        stress_points += 20
    elif avg_meeting_hours > 3:
        stress_points += 10
...
def _calculate_snapshot_metrics(
    events: List[CalendarEvent], window_days: int
) -> Dict[str, float]:
    total_seconds = sum((e.end_time - e.start_time).total_seconds() for e in events)
    meeting_seconds = sum(
        (e.end_time - e.start_time).total_seconds() for e in events if e.is_meeting
    )
    focus_seconds = sum(
        (e.end_time - e.start_time).total_seconds()
        for e in events
        if not e.is_meeting and (e.end_time - e.start_time).total_seconds() >= 2700
    )

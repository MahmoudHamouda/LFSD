...
class GoogleFitService:
    ...
    def __init__(self, db: Session):
        ...
        self.redirect_uri = (
            self.settings.APP_BASE_URL + "/health/google/callback"
            if hasattr(self.settings, "APP_BASE_URL")
            else "http://localhost:3000/health/google/callback"
        )
    ...
    async def get_valid_token(self, connection: Connection) -> str:
        ...
        if not expires_at or expires_at < datetime.now(timezone.utc) + timedelta(
            minutes=5
        ):
    ...
    async def _refresh_token(self, connection: Connection, old_creds: Dict) -> Dict:
        ...
        if not refresh_token:
            logger.error(
                f"Refresh failed: No refresh token for user {connection.user_id}"
            )
        ...
        try:
            ...
            new_creds = {
                "access_token": data.get("access_token"),
                "refresh_token": data.get("refresh_token")
                or refresh_token,  # CRITICAL: Guard against overwrite
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.post(
                    f"{self.API_BASE}/dataset:aggregate", headers=headers, json=body
                )
                response.raise_for_status()
                data = response.json()
    ...
    async def _process_aggregated_data(self, user_id: str, buckets: List[Dict]):
        ...
        for bucket in buckets:
            ...
            date_normalized = datetime.fromtimestamp(
            ...
            for dataset in bucket.get("dataset", []):
                ...
                for p in points:
                    ...
                    if "step_count" in dtype:
                        ...
                        if hr_val > 0:
                            daily_metrics["heart_rates"].append(hr_val)

            # Calculate final biometrics
            avg_hr = (
                sum(daily_metrics["heart_rates"]) / len(daily_metrics["heart_rates"])
                if daily_metrics["heart_rates"]
                else 0
            )

            # Atomic Upsert logic bounded by User + Date + Source
            # Normalizing to DateTime for the model while keeping 'date' semantic
            dt_day = datetime.combine(
                date_normalized, datetime.min.time(), tzinfo=timezone.utc
            )

            existing = (
                self.db.query(HealthDataSample)
                .filter(

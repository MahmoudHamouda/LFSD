...
class AppleHealthService:
    ...
    def _generate_client_secret(self) -> str:
        ...
        if not all([self.private_key, self.key_id, self.team_id, self.client_id]):
            raise ValueError(
                "Incomplete Apple developer credentials for secret generation."
            )
    ...
    async def exchange_code(
        self, code: str, user_id: str, state: Optional[str] = None
    ) -> Connection:
        """
        ...
        try:
            ...
            AuditService.log_audit(
                ...
                metadata={
                    "provider": self.PROVIDER,
                    "simulated": "sim" in data["access_token"],
                },
            )
    ...
    def ingest_simulated_health_data(self, user_id: str, days: int = 14):
        ...
        if self.settings.ENV == "prod":
            logger.error(
                f"PROD ALERT: Attempted to run simulation for user {user_id}. Blocked."
            )
            return
    ...
    def _upsert_health_sample(
        self, user_id: str, date_dt: datetime, metrics: Dict[str, Any]
    ):
        """
        ...
        if existing:
            ...
            existing.calories_kcal = metrics.get(
                "calories_kcal", existing.calories_kcal
            )
            existing.active_minutes = metrics.get(
                "active_minutes", existing.active_minutes
            )
            existing.avg_hr_bpm = metrics.get("avg_hr_bpm", existing.avg_hr_bpm)
            existing.sleep_minutes = metrics.get(
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                f"Health sample upsert failed for {user_id} on {clean_date}: {e}"
            )

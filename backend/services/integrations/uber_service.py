...
class UberService:
    ...
    def _get_headers(self, access_token: Optional[str] = None) -> Dict[str, str]:
        ...
        if access_token:
            ...
            raise ValueError(
                "No valid Uber credentials available (Access Token or Server Token required)"
            )
    ...
    def _get_user_token(self, user_id: str) -> Optional[str]:
        ...
        conn = (
            ...
            .filter(Connection.user_id == user_id, Connection.provider == "uber")
            .first()
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Uber API Error (Estimates): {response.status_code} - {response.text}"
                    )
                    return {
    ...
    async def book_ride(
        ...
        if not token:
            return {
                "error": "User is not connected to Uber. Please connect your account."
            }
        ...
        try:
            payload = {
                ...
                "start_latitude": start_location.get("lat")
                or start_location.get("latitude"),
                "start_longitude": start_location.get("lng")
                or start_location.get("longitude"),
                "end_latitude": end_location.get("lat") or end_location.get("latitude"),
                "end_longitude": end_location.get("lng")
                or end_location.get("longitude"),
            }
            if fare_id:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 202]:
                    ...
                    logger.error(
                        f"Uber Booking Error: {response.status_code} - {response.text}"
                    )
    ...
    def format_price_response(self, api_response: Dict[str, Any]) -> str:
        ...
        if api_response.get("error"):
            ...
            return (
                f"Unable to fetch Uber prices at the moment. ({api_response['error']})"
            )
        ...
        for price in prices[:4]:
            name = price.get(
                "localized_display_name", price.get("display_name", "Ride")
            )

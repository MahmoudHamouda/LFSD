...
def compute_health_score(
    user_id: str, db: Session, override_input: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        inputs = {
            ...
            # B) Activity
            "activity_level": get_profile_val(
                "activity_level", "activity_level", "Moderate"
            ),
            "activity_type": onboarding.get("activity_type", "None"),
            # C) Recovery
            "stress_level": get_profile_val(
                "stress_level", "stress_level", "Sometimes"
            ),
            "energy_pattern": get_profile_val(
                "energy_pattern", "energy_pattern", "Stable"
            ),
            # D) Nutrition
        ...
        def parse_sleep_hours(range_str):
            ...
            if ">9" in range_str:
                return 9.5
            try:
                return float(range_str)  # Fallback if direct number passed
            except:
                return 7.5
        ...
        val_hours = (
            avg_sleep_min / 60
            if avg_sleep_min > 0
            else parse_sleep_hours(inputs["sleep_hours_str"])
        )

        score_duration = 0
        if 7 <= val_hours <= 9:
            score_duration = 15
        elif 6 <= val_hours < 7 or 9 < val_hours <= 10:
            score_duration = 10
        elif 5 <= val_hours < 6:
            score_duration = 5
        else:
        ...
        if avg_active > 0:
            ...
            if atype in ["Running", "Sports", "Gym"]:
                score_int = 10
            elif atype in ["Yoga", "Walking"]:
                score_int = 6
            elif atype == "None":
                score_int = 0
            else:
        ...
        if rhr_meta.get("value", 0) > 0:
            ...
            if rhr_meta["value"] < 60:
                recovery_score = min(20, recovery_score + 2)
            elif rhr_meta["value"] > 80:
                recovery_score = max(0, recovery_score - 2)

        recovery_conf = rhr_meta.get(
            "confidence", 0.9 if rhr_meta.get("value", 0) > 0 else 0.4
        )

        # --- Pillar 4: Nutrition & Habits (20%) ---
        score_diet = 0
        ds = inputs.get("diet_style", "Balanced")
        if ds in ["Balanced", "Mediterranean"]:
            score_diet = 6
        elif ds in ["Vegetarian", "Vegan", "High-protein"]:
            score_diet = 5
        elif ds == "High-carb":
            score_diet = 3
        else:
        ...
        overall_conf = (
            sleep_conf + movement_conf + recovery_conf + nutrition_conf + lifestyle_conf
        ) / 5
        ...
        result = {
            ...
            "dimensions": {
                "sleep": {
                    ...
                    "confidence": round(sleep_conf, 2),
                    "max": 25,
                },
                "movement": {
                    "score": round(movement_score, 1),
                    "confidence": round(movement_conf, 2),
                    "max": 25,
                },
                "recovery": {
                    "score": round(recovery_score, 1),
                    "confidence": round(recovery_conf, 2),
                    "max": 20,
                },
                "nutrition": {
                    "score": round(nutrition_score, 1),
                    "confidence": round(nutrition_conf, 2),
                    "max": 20,
                },
                "lifestyle": {
                    "score": round(lifestyle_score, 1),
                    "confidence": round(lifestyle_conf, 2),
                    "max": 10,
                },
            },
        }
...
def calculate_health_score(
    user_id: str, onboarding_data: Dict[str, Any], db: Session
) -> Dict[str, Any]:

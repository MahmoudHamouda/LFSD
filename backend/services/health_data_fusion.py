...
def _calculate_confidence(sources: List[str], metric_type: str, sample_count: int = 0) -> float:
    """
    ...
    if metric_type in ["sleep", "recovery"]:
        if has_whoop: base_conf = 0.95
        elif has_wearable: base_conf = 0.85
        else: base_conf = 0.5
        
    elif metric_type == "activity":
        if has_wearable: base_conf = 0.90
        elif has_whoop: base_conf = 0.85
        else: base_conf = 0.4

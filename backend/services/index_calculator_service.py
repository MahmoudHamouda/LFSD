...
def calculate_financial_wellbeing_index(user_id: str, db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        if surplus_ratio < 0:
            cashflow_score = max(0, 50 + (surplus_ratio * 100)) # Rapid drop if negative
        else:
        ...
        weighted_total = (savings_score * 0.4) + (cashflow_score * 0.4) + (debt_score * 0.2)
...
def calculate_time_saved_index(user_id: str, db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        total_mins_saved = (count_automation * 5) + (count_optimization * 2) + (count_delegation * 1)
...
def calculate_health_index(user_id: str, db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        ).order_by(HealthDailySummary.date.desc()).limit(7).all()
        ...
        steps_score = min(steps / 10000.0, 1.2) * 80 # Up to 120% target, but capped at logic stage
        ...
        if len(summaries) >= 3:
            ...
            if avg_steps > 5000: consistency_bonus = 10

...
class SkyscannerService:
    ...
    async def search_flights(
        ...
        if not self.api_key or self.api_key == "mock":
            logger.info(f"Using mock flight data for {origin} -> {destination} on {departure_date.date()}")
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout, headers=self.headers) as client:
                ...
                response = await client.get(f"{self.BASE_URL}/search-one-way", params=search_params)
    ...
    def _validate_input(self, origin: str, destination: str, date: datetime, currency: str):
    ...
    def _get_mock_flights(self, origin: str, destination: str, date: datetime, currency: str) -> List[Dict[str, Any]]:
        """
        ...
        base_date = date.replace(tzinfo=timezone.utc) if date.tzinfo else date.replace(tzinfo=timezone.utc)
    ...
    def _transform_api_response(self, data: Dict[str, Any], currency: str) -> List[Dict[str, Any]]:
        """

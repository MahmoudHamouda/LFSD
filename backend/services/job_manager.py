...
class JobManager:
    ...
    def create_job(self, job_type: str, user_id: Optional[str] = None, source: str = "system") -> str:
    ...
    def update_job(
        ...
        try:
            ...
            if job.status in [JobStatus.COMPLETED, JobStatus.FAILED] and status == JobStatus.PROCESSING:
                logger.warning(f"Invalid transition from {job.status} to {status} for job {job_id}")
                return
    ...
    def cleanup_old_jobs(self):
        ...
        try:
            now = datetime.utcnow()
            deleted = self.db.query(BackgroundJob).filter(BackgroundJob.expires_at < now).delete()
            self.db.commit()
            if deleted > 0:

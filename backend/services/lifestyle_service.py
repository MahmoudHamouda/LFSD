...
class LifestyleService:
    ...
    def get_upcoming_events(self, user_id: str) -> List[LifestyleEvent]:
        ...
        now = datetime.utcnow()
        return (
            self.db.query(LifestyleEvent)
            .filter(LifestyleEvent.user_id == user_id, LifestyleEvent.start_time >= now)
            .order_by(LifestyleEvent.start_time.asc())
            .all()
        )
    ...
    def create_event(self, user_id: str, event_data: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if start_str.endswith("Z"):
                start_str = start_str[
                    :-1
                ]  # Brittle naive fix, better to use specialized parser but this works for basic Z
            ...
            event = LifestyleEvent(
                ...
                cost_estimated=max(
                    0, float(event_data.get("cost_estimated", 0))
                ),  # Ensure non-negative
                source="manual",
            )
            self.db.add(event)
    ...
    def get_recommendations(self, user_id: str, mood: str = "stress") -> List[dict]:
        ...
        prefs = (
            user.profile_json.get("preferences", {})
        ...
        return [
            {
                ...
                "progress": (
                    int((g.saved_amount / g.target_amount) * 100)
                    if g.target_amount > 0
                    else 0
                ),
                "target_date": g.target_date.isoformat() if g.target_date else None,
                "pillar": g.pillar
                or "finance",  # Use pillar instead of dead 'category'
            }
            for g in goals
        ]
    ...
    def create_goal(self, user_id: str, goal_data: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if goal_data.get("target_date"):
                ...
                target_date_val = datetime.fromisoformat(
                    goal_data.get("target_date").replace("Z", "")
                )

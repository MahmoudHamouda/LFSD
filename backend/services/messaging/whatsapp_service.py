...
class WhatsAppService(BaseMessagingService):
        ...
        **kwargs
        ...
        try:
            ...
            if not msg_body or len(msg_body) > 4096:
                return MessageResponse(success=False, provider=self.provider_name, status="failed", error="Message empty or too long")
            ...
            return MessageResponse(success=False, provider=self.provider_name, status="failed", error=str(e))
    ...
    def validate_signature(self, payload: bytes, signature_header: str) -> bool:
        ...
        mac = hmac.new(self.settings.SECRET_KEY.encode(), msg=payload, digestmod=hashlib.sha256)

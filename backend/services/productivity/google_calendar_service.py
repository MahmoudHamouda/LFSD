...
class GoogleCalendarService(BaseCalendarService):
    ...
    def __init__(
        self, db: Optional[Session] = None, access_token: Optional[str] = None
    ):
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 400:  # invalid_grant
                    ...
                    self.db.commit()
                    raise CalendarAuthError(
                        "Google access revoked or invalid refresh token."
                    )
    ...
    async def create_event(
        ...
        payload = {
            ...
            "start": {
                "dateTime": t_start.isoformat().replace("+00:00", "Z"),
                "timeZone": "UTC",
            },
            "end": {
                "dateTime": t_end.isoformat().replace("+00:00", "Z"),
                "timeZone": "UTC",
            },
        }
    ...
    def _normalize_google_event(self, g_event: Dict[str, Any]) -> NormalizedEvent:
        ...
        busy = (
            BusyStatus.FREE
            if g_event.get("transparency") == "transparent"
    ...
    def _upsert_local_event(self, user_id: str, event: NormalizedEvent):
        ...
        existing = (
            ...
            .filter(
                ...
                CalendarEvent.id
                == event.id,  # Assuming ID matches provider ID for sync
            )
            .first()
        )
    ...
    async def list_calendars(
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            headers = {"Authorization": f"Bearer {token}"}
            response = await client.get(
                f"{self.API_BASE}/users/me/calendarList", headers=headers
            )
            response.raise_for_status()
...
def get_google_calendar_service(
    db: Optional[Session] = None, access_token: Optional[str] = None

...
class OutlookCalendarService(BaseCalendarService):
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                response = await client.get(
                    f"{self.BASE_URL}/me/calendars", headers=headers
                )
    ...
    async def check_availability(
        self, user_id: str, start_time: datetime, end_time: datetime, **kwargs
    ) -> BusyStatus:
        events = await self.list_events(user_id, start_time, end_time, **kwargs)
        max_busy = BusyStatus.FREE
        for e in events:
            if e.busy_status == BusyStatus.BUSY:
    ...
    def _normalize_outlook_event(self, o_event: Dict[str, Any]) -> NormalizedEvent:
        ...
        return NormalizedEvent(
            ...
            status=status_map.get(
                o_event.get("responseStatus", {}).get("response"), EventStatus.CONFIRMED
            ),
            busy_status=busy_status_map.get(
                o_event.get("showAs", "busy").lower(), BusyStatus.BUSY
            ),
            is_all_day=o_event.get("isAllDay", False),
            raw=o_event,
        )

    def _get_mock_normalized_events(
        self, t_min: datetime, t_max: datetime
    ) -> List[NormalizedEvent]:

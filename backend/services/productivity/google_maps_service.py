...
class GoogleMapsService:
            ...
            for attempt in range(2):
                try:
                    ...
                    if status == "OK" and data.get("results"):
                        ...
                        logger.info(f"Geocoding found no results for: [REDACTED ADDRESS]")
                        return None
                    elif status in ["OVER_QUERY_LIMIT", "REQUEST_DENIED"]:
                        logger.error(f"Google Maps API error: {status}. Message: {data.get('error_message')}")
    ...
    def _get_mock_coordinates(self, address: str) -> Tuple[float, float]:
        ...
        if "dubai" in addr or "dxb" in addr:
            if "mall" in addr: return 25.1972, 55.2744
            if "airport" in addr: return 25.2532, 55.3657
            return 25.2048, 55.2708
        elif "cairo" in addr or "egypt" in addr:
            if "pyramid" in addr: return 29.9792, 31.1342
            if "airport" in addr: return 30.1219, 31.4056
    ...
    def _get_mock_distance_matrix(self, num_origins: int, num_destinations: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "destination_addresses": ["Mock Destination (MOCK_MODE)"] * num_destinations
        }

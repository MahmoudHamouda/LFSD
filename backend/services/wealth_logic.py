...
def validate_monthly_allocation(allocation_amount: float, context_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if not isinstance(allocation_amount, (int, float)) or allocation_amount <= 0:
         return {"valid": False, "message": "Allocation amount must be a positive number.", "error_type": "INVALID_INPUT"}

    required_keys = ['monthly_income', 'fixed_expenses']
    missing_keys = [k for k in required_keys if k not in context_data]
    if missing_keys:
        return {"valid": False, "message": f"Missing financial data for validation: {', '.join(missing_keys)}", "error_type": "MISSING_CONTEXT"}
    ...
    try:
        ...
        return {"valid": False, "message": "Financial context data must be numeric.", "error_type": "INVALID_CONTEXT_TYPE"}

    # 2. Logic Validation
    if monthly_income <= 0:
        return {"valid": False, "message": "Monthly income must be positive to validate goals.", "error_type": "ZERO_INCOME"}
    ...
    return {"valid": True, "message": "Goal allocation appears feasible and sustainable."}

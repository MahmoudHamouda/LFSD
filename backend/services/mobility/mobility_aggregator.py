...
class MobilityAggregator:
    ...
    async def compare_prices(
        ...
        for name, svc in to_query.items():
            tasks.append(self._safe_query(name, svc, start_lat, start_lng, end_lat, end_lng, user_id, currency))
    ...
    async def _safe_query(self, name: str, service: BaseMobilityService, *args) -> tuple:
        ...
        try:
            ...
            res = await asyncio.wait_for(service.get_price_estimates(*args), timeout=15.0)
            return (name, res)
        except Exception as e:
            logger.error(f"Aggregator failure for {name}: {e}", exc_info=True)
            return (name, {"success": False, "error": f"Provider {name} unreachable", "prices": []})
    ...
    async def book_ride(
        ...
        **kwargs
        ...
        if idempotency_key:
            ...
            if existing:
                if existing.status == "CONFIRMED":
                    return {"success": True, "ride_id": existing.external_order_id, "is_idempotent": True}
                if existing.status == "PENDING":
                    return {"success": False, "error": "Ride booking is already in progress"}
        ...
        try:
            ...
            provider_kwargs = {k: v for k, v in kwargs.items() if k not in ["db", "idempotency_key"]}
            ...
            if response.get("success"):
                ...
                order.external_order_id = response.get("ride_id") or response.get("ticket_id") or response.get("id")
                
                # Extract cost reliably (handle strings/missing values)
                raw_cost = response.get("estimated_cost") or response.get("details", {}).get("cost") or 0.0
                ...
                if order.amount_estimated > 0:
                    tx = FinancialTransaction(
                        id=str(uuid.uuid4()),
                        user_id=user_id,
                        amount=-abs(order.amount_estimated), # Absolute negative for expense
                ...
                AuditService.log_audit(
                    db=db, actor_id=user_id, action="CONFIRM_ORDER",
    ...
    def _format_comparison(self, results: Dict[str, Any], currency: str) -> Dict[str, Any]:
        all_options = []
        queried_count = 0
        ...
        for provider, data in results.items():
            if data.get("success"):
                ...
                for price in data.get("prices", []):
                    all_options.append({
                        "provider": provider,
                        "ride_type": price.get("localized_display_name") or price.get("display_name"),

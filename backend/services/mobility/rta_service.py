...
class RTAService(BaseMobilityService):
        ...
        for lat, lng, label in [
            ...
            if lat is not None and lng is not None:
                ...
                if err:
                    return {
                        "success": False,
                        "error": f"{label} coordinates invalid: {err}",
                    }

        if end_lat is None or end_lng is None:
            return {
                "success": False,
                "error": "End coordinates are required for price estimation",
            }

        if start_lat == end_lat and start_lng == end_lng:
            return {
                "success": False,
                "error": "Start and end locations cannot be identical",
            }
        ...
        if self.api_key:
            try:
                async with httpx.AsyncClient(timeout=self.timeout) as client:
                    ...
                    return {
                        "success": True,
                        "prices": data.get("options", []),
                        "mock": False,
                    }

            except (httpx.RequestError, httpx.HTTPStatusError) as e:
                logger.error(
                    f"RTA API request failed: {e}",
                    extra={"tags": {"provider": "rta", "endpoint": "cost"}},
                )
    ...
    async def book_ride(
        ...
        **kwargs,
        ...
        if ride_type_normalized not in self.SUPPORTED_TYPES:
            return {
                "success": False,
                "error": f"Unsupported transit type: {ride_type}. Use one of: {self.SUPPORTED_TYPES}",
            }
    ...
    def log_price_history(
        self, user_id: str, prices: List[Dict], start: Dict, end: Dict
    ) -> None:

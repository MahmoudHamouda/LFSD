...
class BillingService:
    ...
    def get_overall_summary(
        ...
        total_revenue = 0.0
        active_subs = (
            self.db.query(Subscription).filter(Subscription.status == "active").all()
        )
        ...
        token_usage_by_model = (
            ...
            .filter(DBMessage.date >= start_date, DBMessage.date <= end_date)
            .group_by(DBMessage.model_used)
            .all()
        )
        ...
        mobility_interactions = (
            self.db.query(MobilityTrip)
            .filter(MobilityTrip.created_at >= start_date)
            .count()
        )
    ...
    def get_customer_reconciliation(self) -> List[Dict[str, Any]]:
        ...
        user_subs = (
            self.db.query(User.id, User.email, Subscription.plan_id)
            .outerjoin(
                Subscription,
                (User.id == Subscription.user_id) & (Subscription.status == "active"),
            )
            .all()
        )
        ...
        ai_stats = (
            ...
            .filter(DBMessage.date >= start_of_month)
            .group_by(DBMessage.user_id, DBMessage.model_used)
            .all()
        )
        ...
        mobility_stats = (
            self.db.query(
                MobilityTrip.user_id, func.count(MobilityTrip.id).label("trip_count")
            )
            .filter(MobilityTrip.created_at >= start_of_month)
            .group_by(MobilityTrip.user_id)
            .all()
        )
        ...
        return [
            {
                ...
                "unit": "Tokens",
                "usage": self.db.query(
                    func.sum(DBMessage.input_tokens + DBMessage.output_tokens)
                )
                .filter(DBMessage.date >= datetime.utcnow() - timedelta(days=30))
                .scalar()
                or 0,
            },
            {
                ...
                "unit": "Trips",
                "usage": self.db.query(MobilityTrip)
                .filter(
                    MobilityTrip.created_at >= datetime.utcnow() - timedelta(days=30)
                )
                .count(),
            },
        ]

...
def compute_time_score(
    ...
    try:
        ...
        if override_input:
            ...
            if "commute_duration" in override_input:
                ...
                if "commute_minutes_per_day" in metrics:
                    ...
                    metrics["commute_minutes_per_day"] = {
                        "value": val,
                        "confidence": 0.5,
                    }
        ...
        if routine == "I follow a routine":
            p1_score += 8
        elif routine == "I try":
            p1_score += 5
        elif routine == "I'm spontaneous":
            p1_score += 3
        else:
            p1_score += 1  # Not really

        task = profile.get("task_style", "I react to what's urgent")
        if task == "I plan ahead":
            p1_score += 7
        elif task == "I plan but things shift":
            p1_score += 5
        elif task == "I go with the flow":
            p1_score += 3
        else:
            p1_score += 1  # React
        ...
        if isinstance(drains, list):
            if "Meetings" in drains:
                p3_score -= 2
            if "Emails" in drains:
                p3_score -= 2
            if "Last-minute tasks" in drains:
                p3_score -= 3
        ...
        if admin_hrs > 10:
            p4_score -= 8  # Only punish extreme admin
        elif admin_hrs > 5:
            p4_score -= 4
        ...
        if overwhelm == "Almost always":
            p5_score -= 8
        elif overwhelm == "Often":
            p5_score -= 5
        elif overwhelm == "Sometimes":
            p5_score -= 2
        # Rarely = 0 deduction
        ...
        # --- Overall ---
        overall_score = (
            structure_score + load_score + focus_score + friction_score + stress_score
        )
        overall_conf = (
            structure_conf + load_conf + focus_conf + friction_conf + stress_conf
        ) / 5

...
class GrowthService:
    ...
    def get_entitlements(user_id: str, db: Session) -> EntitlementResponse:
        ...
        if subscription:
            ...
            try:
                ...
                logger.warning(
                    f"Invalid plan_id {subscription.plan_id} for user {user_id}, falling back to FREE"
                )
                plan_id = PlanId.FREE
        ...
        if tier:
            ...
            base_config = copy.deepcopy(
                PLAN_CONFIGS.get(plan_id, PLAN_CONFIGS[PlanId.FREE])
            )

        # 3. Apply User-Specific Overrides (Validated)
        override = (
            db.query(UserLimitOverride)
            .filter(
            ...
            for key, val in override.overrides_json.items():
                if key in base_config["limits"]:
                    ...
                    if isinstance(val, int) and (val >= -1):
                        ...
                        logger.warning(
                            f"Invalid override value {val} for key {key} user {user_id}"
                        )
        ...
        # Goals: Total goals
        usage["goals"] = db.query(LifeGoal).filter(LifeGoal.user_id == user_id).count()
        ...
        token_stats = (
            ...
            .filter(DBMessage.user_id == user_id, DBMessage.date >= first_of_month)
            .first()
        )
    ...
    @staticmethod
    def create_or_upgrade_subscription(

...
def seed_rich_time_data(user_id):
    ...
    db.add(LifeGoal(
        ...
        saved_amount=12, priority="medium", pillar="time",
        type="productivity"
    ))
...
def seed_minimal_health_data(user_id):
    ...
    for i in range(14):
        db.add(HealthDailySummary(
            user_id=user_id, date=(datetime.now(timezone.utc) - timedelta(days=i)).date(),
...
def populate_synthetic_data():
    ...
    try:
        ...
        user2 = create_base_user(
            "user-finance", "finance@helm.com", "Finance User", "Planner", "Focused on wealth building."
        )
        ...
        user3 = create_base_user(
            "user-health", "health@helm.com", "Health User", "Athlete", "Marathon training."
        )
        ...
        user4 = create_base_user(
            "user-time", "time@helm.com", "Time User", "Productivity Expert", "Calendar optimization master."
        )
        ...
        user5 = create_base_user(
            "user-super", "super@helm.com", "Super User", "Legend", "Excelling in all areas."
        )

...
class IntegrationRegistry:
    ...
    def register(cls, intent_name: str):
        ...
        def wrapper(executor_cls: Type[BaseExecutor]):
            ...
            logger.info(
                f"Registered Executor: {executor_cls.__name__} for Intent: {intent_name}"
            )
            return executor_cls
    ...
    def get_executor(cls, intent_name: str) -> Optional[BaseExecutor]:
        ...
        try:
            ...
            logger.error(
                f"Failed to instantiate executor for {intent_name}: {e}", exc_info=True
            )
...
class DegradedExecutor(BaseExecutor):
    ...
    async def execute_safe(
        ...
        return {
            "status": "error",
            "error_message": f"Integration offline: {self.error_msg}",
        }

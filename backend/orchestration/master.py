...
class Orchestrator:
    ...
    async def process_message(
        self, text: str, user_id: str, context: Dict[str, Any], history: list = None
        ...
        if intent.name in ["MOBILITY"]:
            ...
            await self._log_audit(
                ...
                success=result.get("executor_found", False)
            ...
            if status == "success" and result.get("data"):
                human_answer = await self.composer.compose(
                    intent.name, result["data"], context
                )
                ...
                await self._log_activity(
                    user_id,
                    "Route Computed",
                    f"Found commute options from {origin} to {destination}",
                )
                ...
                return result.get(
                    "message", f"The {intent.name} service is currently unavailable."
                ), {"is_orchestrator": True, "intent": intent.name, "status": status}
            else:
                # Error or unexpected status
                return result.get(
                    "message", "Something went wrong. Please try again."
                ), {"is_orchestrator": True, "intent": intent.name, "status": "error"}

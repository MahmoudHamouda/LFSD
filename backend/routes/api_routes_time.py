...
async def save_manual_time_profile(
    user_id: str, profile_data: Dict[str, Any], db: Session = Depends(get_db)
):
    ...
    allowed_keys = [
        "work_status",
        "work_hours_per_week",
        "uses_digital_calendar",
        "commute_duration",
        "time_meals_house_daily",
        "time_admin_weekly",
        "routine_style",
        "task_style",
...
async def upload_calendar_screenshot(
    ...
    try:
        ...
        if not text or len(text) < 10:
            raise HTTPException(
                status_code=400,
                detail="Could not extract text from image. Please ensure it is clear.",
            )
        ...
        gemini = GeminiService(
            db
        )  # We might need to adjust init if it does heavy loading
            ...
            try:
                ...
                event = TimeEvent(
                    ...
                    start_time=datetime.fromisoformat(
                        e_data["start_time"].replace("Z", "+00:00")
                    ),
                    end_time=datetime.fromisoformat(
                        e_data["end_time"].replace("Z", "+00:00")
                    ),
        ...
        return {
            "status": "success",
            "message": f"Parsed {count} events from screenshot",
        }
...
async def get_time_events(
    ...
    events = (
        db.query(CalendarEvent).filter(CalendarEvent.user_id == current_user.id).all()
    )
...
async def get_time_score(user_id: str, db: Session = Depends(get_db)):
    ...
    if dims["structure"]["score"] < 15:
        recommendations.append(
            "High Impact: Try putting your key tasks in a simple daily calendar block."
        )
    if dims["load"]["score"] < 15:
        recommendations.append(
            "Your days are very full. Consider protecting 1–2 meeting-free hours."
        )
    if dims["friction"]["score"] < 10:
        recommendations.append(
            "You spend a lot of time on admin. We can help automate bills & errands."
        )
    if dims["stress"]["score"] < 10:
        recommendations.append(
            "You often feel short on time. Let's start by freeing 30–60 minutes a week."
        )

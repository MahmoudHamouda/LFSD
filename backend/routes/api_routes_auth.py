...
async def signup(payload: SignupPayload, db: Session = Depends(get_db)):
    try:
        ...
        if existing_user:
            ...
            raise HTTPException(
                status_code=409, detail="Account already exists. Please log in."
            )
...
async def google_auth_url(response: Response, db: Session = Depends(get_db)):
    ...
    response.set_cookie(
        ...
        httponly=True,
        samesite="lax",
        secure=core.config.get_settings().ENV
        == "production",  # Conditional based on env
...
async def google_auth_callback(
    payload: CodePayload, request: Request, db: Session = Depends(get_db)
):
    # Verify state
    cookie_state = request.cookies.get("oauth_state")
    if not cookie_state or not payload.state or cookie_state != payload.state:
        print(
            f"[AUTH] CSRF State Mismatch. Cookie: {cookie_state}, Payload: {payload.state}"
        )
        raise HTTPException(
            status_code=400, detail="Invalid state parameter. Possible CSRF attempt."
        )
...
async def forgot_password(
    payload: ForgotPasswordPayload, db: Session = Depends(get_db)
):

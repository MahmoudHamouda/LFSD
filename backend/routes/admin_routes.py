...
async def unlock_user(
    *,
    ...
    try:
        ...
        return {"status": "success", "user_id": user.id, "account_status": user.account_status}
...
async def update_tier(
    ...
    if not tier:
        ...
        tier = TierConfig(plan_id=plan_id, name=payload.name or plan_id, config_json=payload.config_json)
        db.add(tier)
    else:
        if payload.name: tier.name = payload.name

...
def create_health_connection(
    user_id: str, provider: str, permissions: List[str], db: Session
):
...
def update_connection_status(
    connection_id: str, status: str, db: Session, error_message: Optional[str] = None
):
...
def store_health_metric(
    ...
    # 3. Recovery/HRV -> HealthDailySummary
        ...
        summary = (
            ...
            .filter(
                HealthDailySummary.user_id == user_id, HealthDailySummary.date == date
            )
            .first()
        )
        ...
        if not summary:
            summary = HealthDailySummary(
                id=str(uuid.uuid4()), user_id=user_id, date=date
            )
            db.add(summary)
...
async def get_health_connections(
    ...
    connections = (
        db.query(Connection)
        .filter(
            Connection.user_id == user_id, Connection.provider.in_(health_providers)
        )
        .all()
    )
        ...
        "connections": [
            {
                ...
                "status": conn.status,
                "permissions": (
                    json.loads(conn.metadata_json).get("permissions", [])
                    if conn.metadata_json
                    else []
                ),
                "connectedAt": (
                    conn.updated_at.isoformat() if conn.status == "connected" else None
                ),
                "lastSyncedAt": conn.updated_at.isoformat(),
                "errorMessage": json.loads(conn.metadata_json or "{}").get(
                    "last_error"
                ),
            }
...
async def connect_health_provider(
    ...
    existing = (
        db.query(Connection)
        .filter(Connection.user_id == user_id, Connection.provider == request.provider)
        .first()
    )
    ...
    if existing:
        ...
        connection = create_health_connection(
            user_id, request.provider, request.permissions, db
        )
...
async def get_health_summaries(
    ...
    """
    query = db.query(HealthDailySummary).filter(
        HealthDailySummary.user_id == current_user.id
    )

    if start_date:
        query = query.filter(
            HealthDailySummary.date >= datetime.strptime(start_date, "%Y-%m-%d").date()
        )
    if end_date:
        query = query.filter(
            HealthDailySummary.date <= datetime.strptime(end_date, "%Y-%m-%d").date()
        )
        ...
        try:
            ...
            if not existing:
                existing = HealthDailySummary(
                    id=str(uuid.uuid4()), user_id=current_user.id, date=date_obj
                )
                db.add(existing)
            ...
            if "sleep_quality" in d:
                existing.sleep_quality_score = d["sleep_quality"]
            if "hrv" in d:
                existing.hrv_average = d["hrv"]
            if "rhr" in d:
                existing.resting_heart_rate = d["rhr"]
            if "steps" in d:
                existing.steps_count = d["steps"]
            if "active_minutes" in d:
                existing.active_minutes = d["active_minutes"]
...
async def track_interest(
    ...
    interest = FeatureInterest(user_id=current_user.id, feature_name=feature)
    db.add(interest)
    db.commit()
...
async def get_user_health_score(
    ...
    if user_id != current_user.id:
        ...
        raise HTTPException(
            status_code=403, detail="Forbidden: You can only view your own score."
        )

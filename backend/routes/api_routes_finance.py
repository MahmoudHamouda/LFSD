    ...
    try:
        ...
        for file_data in files_data:
            ...
            result = await service.process_statement(
                user_id=current_user.id, file_content=file_bytes, filename=filename
            )
...
async def get_current_scores(
    ...
    latest = (
        ...
        .filter(FinancialScore.user_id == current_user.id)
    ...
    try:
        ...
        return {
            ...
            "expected_unlock_date": (
                datetime.utcnow() + timedelta(days=max(0, required - days_with_data))
            ).strftime("%Y-%m-%d"),
        }
    except Exception as e:
        logger.error(f"ERROR in coverage: {e}", exc_info=True)
        return {"category_id": category_id, "error": str(e), "chart_unlocked": False}

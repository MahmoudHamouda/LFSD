...
async def get_price_history(
    ...
    trips = (
        query.order_by(MobilityTrip.id.desc()).limit(limit).all()
    )  # No timestamp directly on Trip?
    ...
    return {"total": len(results), "prices": results}
...
async def get_popular_routes(
    ...
    routes = (
        ...
        .filter(MobilityTrip.user_id == user_id)
        .group_by(
            MobilityTrip.origin_lat,
            MobilityTrip.origin_lon,
            MobilityTrip.destination_lat,
        ...
        .order_by(func.count(MobilityTrip.id).desc())
        .limit(10)
        .all()
    )
...
async def get_provider_usage(
    ...
    provider_stats = (
        db.query(MobilityTrip.provider, func.count(MobilityTrip.id).label("count"))
        .filter(MobilityTrip.user_id == user_id)
        .group_by(MobilityTrip.provider)
        .all()
    )

    return {
        "providers": [
            {"provider": p.provider, "interaction_count": p.count}

...
def get_latest_health_metrics(user_id: str, db: Session) -> HealthMetrics:
    ...
    for metric_type in ["sleep", "recovery", "activity", "hrv", "steps"]:
        latest = (
            ...
            .order_by(HealthDataSample.timestamp.desc())
            .first()
        )
...
async def get_user_profile(
    ...
    user = User(
        ...
        engagement=UserEngagementProfile(
            ...
            streaks=UserStreaks(
                dailyCheckIn=check_in_streak, financialReview=financial_streak
            ),
            mostUsedJourneys=[],
        ),
...
async def update_current_user(
    updates: UserUpdateRequest, db: Session = Depends(get_db)
):
    """

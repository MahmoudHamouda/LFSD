...
async def check_session(
    ...
    current_user: User = Depends(
        get_current_user
    ),  # This relies on Authorization header usually
    ...
    logger.debug(
        f"DEBUG: /session checking user {current_user.id}. Status from DB: {current_user.onboarding_status}"
    )
    ...
    return SessionResponse(
        ...
        user={
            ...
            "name": (
                current_user.profile_json.get("name")
...
async def login(
    payload: SessionLoginRequest, response: Response, db: Session = Depends(get_db)
):
    raise HTTPException(
        status_code=410, detail="Native login is deprecated. Use Auth0."
    )
...
async def complete_onboarding(
    ...
    logger.debug(
        f"DEBUG: /complete called for user {current_user.id}. Current status: {user_to_update.onboarding_status if user_to_update else 'NOT FOUND'}"
    )
    ...
    if user_to_update:
            ...
            if payload.plan_id in [PlanId.FREE, PlanId.PLUS, PlanId.PRO]:
                ...
                if sub:
                    if sub.plan_id != payload.plan_id:
                        logger.info(
                            f"Upgrading user {user_id} from {sub.plan_id} to {payload.plan_id}"
                        )
                    ...
                    logger.info(
                        f"Creating new subscription for {user_id}: {payload.plan_id}"
                    )
                    new_sub = Subscription(
                        user_id=user_id, plan_id=payload.plan_id, status="active"
                    )
                    db.add(new_sub)
        ...
        logger.debug(
            f"DEBUG: /complete committed. New status: {user_to_update.onboarding_status}"
        )

...
async def list_history(
    ...
    conversations = (
        ...
        .order_by(DBConversation.date.desc())
        .all()
    )
    return [
...
async def read_history(request: HistoryReadRequest, db: Session = Depends(get_db)):
    conversation = (
        db.query(DBConversation)
        .filter(DBConversation.id == request.conversation_id)
        .first()
    )
    if not conversation:
        return {"messages": []}
...
async def generate_history(
    ...
    try:
        ...
        debug_logger.info(
            f"Processing generation for conversation: {conversation_id}, messages: {len(messages_data)}"
        )
        ...
        if not conversation_id:
            ...
            if messages_data and len(messages_data) > 0:
                ...
                title = (
                    first_content[:30] if isinstance(first_content, str) else "New Chat"
                )
...
async def message_feedback(
    request: MessageFeedbackRequest, db: Session = Depends(get_db)
):
